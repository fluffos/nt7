...
delete("quest/天龙八部/凌波微步篇", 	me);
 ...
 set(QUESTDIR+"combat_exp",query("combat_exp",  me), 	me);	
	...
	if( query("per", me)>26 )
	  {
		...
		tell_room(environment(me),HIC"转头却见"+query("name", me)+"也是一复神色怪怪的样子，也似乎被什么吸引住了似的。\n"NOR,({me}));
	  ...
	  {
		command("say"+query("name", me)+"，这位姐姐，你且仔细看看这玉像，真是太神色动人了。");
		  command("points");
		  tell_object(me,HIC"\n你侧过身子看那玉像时，只见她眼光跟着转将过来，便似活了一般。你大吃一惊，侧头向右，玉像的眼光似乎也对着你移动。不论\n"
			  "你站在那一边，玉像的眼光始终向着你，眼光中的神色更是难以捉摸，似喜似爱，似是情意深挚，又似黯然神伤。\n"NOR);
		tell_room(environment(me),HIC"转头却见"+query("name", me)+"也是一复神色怪怪的样子，也似乎被什么吸引住了似的。\n"NOR,({me}));
  ...
  {
 	command("say"+query("name", me)+"，也是同道中人，你且仔细看看这玉像，真是太神色动人了。");
	  command("points");
	  tell_object(me,HIC"\n你侧过身子看那玉像时，只见她眼光跟着转将过来，便似活了一般。你大吃一惊，侧头向右，玉像的眼光似乎也对着你移动。不论\n"
	  "你站在那一边，玉像的眼光始终向着你，眼光中的神色更是难以捉摸，似喜似爱，似是情意深挚，又似黯然神伤。\n"NOR);
	tell_room(environment(me),HIC"转头却见"+query("name", me)+"也是一复神色怪怪的样子，也似乎被什么吸引住了似的。\n"NOR,({me}));
	  tell_object(me,HIC"\n你又向玉像呆望良久，这才转头，见东壁上刮磨平整，刻着数十行字，都是“庄子”中的句子，大都出自“逍遥游”、“养生主”、\n"
		  "“秋水”、“至乐”几篇，笔法飘逸，似以极强腕力用利器刻成，每一笔都深入石壁几近半寸。文末题着一行字云：“逍遥子为秋\n"
		  "水妹书。洞中无日月，人间至乐也。”！\n"NOR);
      if( query("gender", me) == "无性")tell_object(me,HIR"\n你暗想：“我要是真能做个女人，就是死一千遍，一万遍，也如身登极乐，欢喜无限。”\n"NOR);
...
addn("mana", me->query_skill("force")/3, 		me);
	...
	{
		command("say"+query("name", me)+"已经是第二次相救了，真的非常感激，我这里正好有块矿石，你且拿去。");
		obj=new("/d/zhiye/obj/kuangshi1");
		obj->set_level(5+random(3));
		obj->move(me);
		message_vision("$N给了$n一块"+query("name", obj)+"。\n",ob,me);
