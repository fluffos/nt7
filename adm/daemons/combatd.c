...
mixed attack_damage(object me, object victim, string attack_skill, object weapon, string limb, mapping action, string *att, int attack_type)
{
       ...
       if( martial_skill == "xuedao-daofa" && me->query_skill("xueying-dafa", 1) >= 100 )
        ...
        while( damage > 0)
        {
                ...
                {
                        damage_info += HIY "$N" HIY "默运战神无敌神技，对$n" HIY "的攻击爆发额外的五成之力。\n" NOR;
                        damage += damage / 2;
                }

                else if( me->query_viplevel() == 8 )
                {
                        damage_info += HIY "$N" HIY "默运战神无敌神技，对$n" HIY "的攻击爆发额外的三成之力。\n" NOR;
                        damage += damage * 3 / 10;
                }
...
varargs mixed inflict_damage(object me, object victim, string attack_skill, int damage, object weapon)
{
        ...
        if( damage > 0 ) {
                ...
                */
                ...
                {
                        damage_info += HIM "$n" HIM "默运玄武护体神技，巨大的玄武虚影突现在身前，消弱了$N" HIM "的五成攻击。\n" NOR;
                        damage -= damage / 2;
                }

                else if( victim->query_viplevel() == 8 )
                {
                        damage_info += HIM "$n" HIM "默运玄武护体神技，巨大的玄武虚影突现在身前，消弱了$N" HIM "的三成攻击。\n" NOR;
                        damage -= damage * 3 / 10;
                }
                         ...
                         int damage, int percent, mixed final, mapping fmsk)
{
        ...
        */
        ...
        {
                damage = 0;
                msg += HIW "$n" HIW "用力一跺足，大地八面龟裂，借着" NOR YEL "土行元磁力"HIW"将所有伤害转移到了厚土之下 ……\n" NOR;
        }
        ...
        */
        ...
        while (damage > 0)
        {
              if( target->is_busy() ) target->interrupt_me(target, 100 + random(50));
                ...
                if( me->query_viplevel() == 9 )
                {
                        msg += HIY "$N" HIY "默运战神无敌神技，对$n" HIY "的攻击爆发额外的五成之力。\n" NOR;
                        damage += damage / 2;
                }

                else if( me->query_viplevel() == 8 )
                {
                        msg += HIY "$N" HIY "默运战神无敌神技，对$n" HIY "的攻击爆发额外的三成之力。\n" NOR;
                        damage += damage * 3 / 10;
                }
                ...
                {
                        msg += HIM "$n" HIM "默运玄武护体神技，巨大的玄武虚影突现在身前，消弱了$N" HIM "的五成攻击。\n" NOR;
                        damage -= damage / 2;
                }

                else if( target->query_viplevel() == 8 )
                {
                        msg += HIM "$n" HIM "默运玄武护体神技，巨大的玄武虚影突现在身前，消弱了$N" HIM "的三成攻击。\n" NOR;
                        damage -= damage * 3 / 10;
                }
