...
mixed ask_hui()
{
	...
	string msg;
        ...
        if( query("can_perform/liuyang-zhang/hui", me) )
                return "我不是已经将这招传授你了吗，怎么，那么快就忘了？";

        if( query("family/family_name", me) != query("family/family_name") )
                return RANK_D->query_respect(me) + "和本派素无瓜葛，何出此言？";

	// 如未完成解珍笼棋局的任务，则开始分配任务
        if( !query("can_perform/liuyang-zhang/hui-quest_end", me) )
	{
		// 如果已领取任务
                if( query("can_perform/liuyang-zhang/hui-step1", me))
		        return "珍笼棋局你解开了吗？老夫等着你的好消息呢。\n";

		command("yi");
		msg = HIG "无崖子对你说道：“你是从何处知道逍遥派还有此绝学，老夫不得而知，但是，这招只传"
		          "与逍遥派有缘弟子，只要你能解开这珍笼棋局，灰飞湮灭便可传你 ……\n" NOR;
                msg += HIC "说罢，无崖子从怀中取出一张棋谱，与你讲解其中概要。\n" NOR;
                
                // chess 小于200LV则无法看懂
                if( me->query_skill("chess", 1) < 200 )
                {
                	msg += HIR "忽然间，你发现自己对弈棋之道一知半解，听得云里雾里 ……\n" NOR;
                	msg += HIG "无崖子摇了摇头对你道：真是可惜，看来，逍遥派这绝学与你无缘了。"
                	           "说罢，长袖一拂，将棋谱收入怀中，不再理会你。\n" NOR;
			...
			return 1;
                ...
                msg += HIC "你越听越觉得这珍笼棋局甚是奇妙，看似有路却无路，看似无路却有路。一时间竟醉思于其中 ……\n" NOR;
                msg += HIG "无崖子对你说道：你可要把这棋谱记好了，倘若它日你能有幸解得开这棋局，便是与我有缘之人，"
                           "逍遥派之绝学定当倾囊相授。说罢，将棋谱收入怀中，不再理会你。\n" NOR;
		
	        msg += HIY "你早已默默将棋谱记住，寻思，这世上真有人能解开这玄妙的棋局？\n" NOR;
	...
	command("nod");
	command("say 不错，不错，竟能解开这千古珍笼棋局，你我乃有缘之人，在我有人之年又得一可塑之材 ……");
	command("haha");
	command("say 既然如此，这逍遥派失传已久的绝学便传授于你，你可要专心听好了。");
	
        if( me->query_skill("liuyang-zhang", 1) < 800 )
                return "你天山六阳掌火候不足，恐怕也难以领会此招。";

        if( me->query_skill("zhemei-shou", 1) < 800 )
                return "你逍遥折梅手火候不足，恐怕也难以领会此招。";
                
        if( me->query_skill("lingbo-weibu", 1) < 600 )
                return "你凌波微步火候不足，恐怕也难以领会此招。";
                                
        if( me->query_skill("martial-cognize", 1) < 800 )
                return "你对武学的领悟还不够，恐怕也难以领会此招。";
        
        if( query("max_neili", me) < 10000 )
        	return "你内力修为不足，难以领会此招。";
        	    
        if( query("family/gongji", me) < 150000 )
                return "你虽在我逍遥门下，但要我传授你此招，也须为门派多做出点贡献，否则如何服众。";

        message_sort(HIY "\n$n" HIY "点了点头，从怀中轻轻取出了一本"
                       "发旧的手稿，指着其中的一段对$N" HIY "不厌其烦的"
                       "细说良久，$N" HIY "一边听一边点头。\n" NOR,
                       me, this_object());

        command("nod");
        command("say 你自己下去领会吧。");
        tell_object(me, HIC "你学会了「灰飞湮灭」。\n" NOR);
