...
{
                int lv = random(6)+10;
                set_name(NOR CYN "玄阴" HIK "聚兽" NOR RED "幡" NOR , ({ "jushou fan", "fan" }));
        set("long", HIC "此乃玄阴教主妖尸谷辰的法宝，以千万生魂和地肺中秽气炼成的黑煞丝合炼而成。\n"
                                    "  共八十一杆布成玄阴炼魂聚魄大阵，可收(shou)摄生魂后放(fang)出去为祸，阴毒无比。\n"
                            HIK "(下线丢失，无法保存)\n" NOR);
                        set_weight(1000);
                set("unit", "杆");
                set("value", 100);
...
//                set("no_uget", 1);
                ...
                set("unique", 1);//已避免分解
                                ...
                                set("wear_msg", NOR CYN "$N" NOR CYN "从怀里掏出八十一面$n" NOR CYN "挂在腰间。\n"); 
                                set("remove_msg", NOR CYN "$N" NOR CYN "把$n" NOR CYN "从腰间解下来放回怀里。\n"); 
