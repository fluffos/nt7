...
#
# Like fluffos/xkx100, this repo's mudlib root IS the repo root itself
# (config.ini: "mudlib directory : ."), so staging is just "every
# git-tracked file except scripts/ and .github/" -- no lib/ subdir to
# select, and no separate gitignored-runtime-dir recreation pass either,
# since every runtime dir here (data/user, data/login, log, temp) already
# ships a tracked nested .gitignore placeholder that git ls-files picks up
# on its own.
#
...
STAGE=$(mktemp -d "${TMPDIR:-/tmp}/nt7pack.XXXXXX")
